function hardWord (input) {
    ...
    missingWords = missingWords.sort((a, b) => b.length - a.length);
    underscores = underscores.sort((a, b) => b.length - a.length);

    for (let i = 0; i < underscores.length; i++) {
        letter = letter.replace(underscores[i], missingWords[i]);
